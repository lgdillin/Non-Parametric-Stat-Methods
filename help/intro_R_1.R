...
num_pos = sum(x>0)
num_tot = length(x)
binom.test(num_pos,num_tot)

## Sign test with paired sample
## Generate another data with a larger mean
y = rnorm(100,3,1)
mean(y)
median(y)
...
p1 <- hist(x)                     # centered at 0
p2 <- hist(y)                     # centered at 3
plot( p1, col=rgb(0,0,1,1/4), freq = F, xlim=c(-5,10))  # first histogram
plot( p2, col=rgb(1,0,0,1/4), freq = F, xlim=c(-5,10), add=T)  # second

### Paired sample sign test
num_pos_diff = sum(y>=x)
num_tot_diff = length(x)
binom.test(num_pos_diff,num_tot_diff,p = 0.5,alternative = "two.sided",conf.level = 0.95)

## Class Exercise 

## The regulations specify that average content of cigarettes may not exceed 
## 16 mg of tar, 1.5 mg of nicotine or 20 mg of carbon monoxide. 

## Test if the 25 barnds in the data adheres to the federal guidelines. Which ones don't?

## Although this is not a situation for hypothesis testing, if we want to do test: 
## Does the median differ significantly? 
